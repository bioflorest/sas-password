var _safeStorage = (function(){
  var _mem = {};
  function _testLS(){
    try { _safeStorage.setItem('__sas_test__','1'); _safeStorage.removeItem('__sas_test__'); return true; }
    catch(e){ return false; }
  }
  var _hasLS = _testLS();
  return {
    getItem: function(k){
      if(_hasLS){ try{ return _safeStorage.getItem(k); } catch(e){} }
      return _mem[k] !== undefined ? _mem[k] : null;
    },
    setItem: function(k,v){
      if(_hasLS){ try{ _safeStorage.setItem(k,v); return; } catch(e){} }
      _mem[k] = v;
    },
    removeItem: function(k){
      if(_hasLS){ try{ _safeStorage.removeItem(k); return; } catch(e){} }
      delete _mem[k];
    }
  };
})();
...
async function _carregarDadosSAS(){
  try {
    ...
    if(resCli.data && resCli.data.length > 0){
      var clientes = resCli.data.map(function(c){
        return { nome: c.nome || '', cpf: c.cpf || '', tel: c.telefone || '', email: c.email || '' };
      });
  ...
  try {
    // Carregar técnicos (tabela equipe)
    var resTec = await supa.from('equipe').select('*').order('nome');
    if(resTec.data && resTec.data.length > 0){
      var tecnicos = resTec.data.map(function(t){
        // campo pode ser 'carga' ou 'cargo' dependendo da versão do banco
        var titulo = t.carga || t.cargo || '';
        return { nome: t.nome || '', titulo: titulo, registro: t.crea_cfb || '' };
      });
      _tecnicos = tecnicos;
      _salvarTecnicosLS();
