...
async function aplicarTodosParametros() {
  ...
  // grava TODAS de uma vez (uma requisição), com dedupe de nomes
  var res = await _supaSalvarLoteParametros(validos);

  // feedback GARANTIDO (alert sempre aparece, não depende de nenhum elemento HTML)
  if (res.ok) {
    alert('✅ ' + res.total + ' cultura(s) salvas no sistema!\n\nContinuam salvas mesmo ao fechar ou atualizar a página. Já valem para as próximas recomendações.');
    _mostrarMsgAplicado('<div style="font-size:1.4rem;margin-bottom:4px;">✅</div><strong>' + res.total + ' cultura(s) salvas no sistema!</strong>', true, true);
  } else {
    alert('❌ Não foi possível salvar no banco.\n\nMotivo: ' + (res.erro || 'erro desconhecido') + '\n\nVerifique sua conexão e tente de novo.');
    _mostrarMsgAplicado('<strong>❌ Não foi possível salvar:</strong> ' + (res.erro || 'erro'), false, true);
  }
}
