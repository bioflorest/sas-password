...
async function aplicarTodosParametros() {
  alert('Passo 1: botão funcionou. Vou começar a salvar.');

  if (!_pdfDadosExtraidos || _pdfDadosExtraidos.length === 0) {
    alert('Passo 2: não há culturas na memória para salvar (_pdfDadosExtraidos vazio). Processe o PDF de novo.');
    return;
  }
  alert('Passo 2: encontrei ' + _pdfDadosExtraidos.length + ' cultura(s) para salvar.');

  if (!window.supa) {
    alert('Passo 3: ERRO - sem conexão com o banco (supa indefinido).');
    return;
  }

  var okCount = 0;
  var falhas = [];
  for (var i = 0; i < _pdfDadosExtraidos.length; i++) {
    var c = _pdfDadosExtraidos[i];
    if (!c || !c.nome) continue;
    _aplicarCulturaNoSistema(c, false);
    var res = await _supaSalvarParametroCultura(c);
    if (res && res.ok) { okCount++; }
    else { falhas.push(c.nome + (res && res.erro ? ' (' + res.erro + ')' : '')); }
  }

  if (falhas.length === 0) {
    alert('PRONTO! ' + okCount + ' cultura(s) salvas no banco com sucesso.');
  } else {
