...
async function aplicarUmParametro(idx) {
  var c = _pdfDadosExtraidos[idx];
  if (!c || !c.nome) return;
  _aplicarCulturaNoSistema(c, false);
  _mostrarMsgAplicado('⏳ Salvando "' + c.nome + '"...', true);
  ...
  if (res.ok) {
    ...
    _mostrarMsgAplicado(
      '<strong>❌ Não foi possível salvar "' + c.nome + '".</strong><br>' +
      '<span style="font-size:.82rem;">' + (res.erro || 'Erro desconhecido.') + '</span>',
      false
    );
  }
}
...
async function aplicarTodosParametros() {
  ...
  // 1) aplica na memória (sem gravar individualmente: 2º arg = false)
  var validos = [];
  _pdfDadosExtraidos.forEach(function(c) {
    if (c && c.nome) { _aplicarCulturaNoSistema(c, false); validos.push(c); }
  });

  // 2) feedback de "salvando..." enquanto grava
  _mostrarMsgAplicado('⏳ Salvando ' + validos.length + ' cultura(s) no banco...', true);

  // 3) grava TODAS de uma vez (uma requisição)
  var res = await _supaSalvarLoteParametros(validos);

  if (res.ok) {
    _mostrarMsgAplicado(
      '<div style="font-size:1.4rem;margin-bottom:4px;">✅</div>' +
      '<strong>' + res.total + ' cultura(s) salvas no sistema!</strong><br>' +
      '<span style="font-size:.82rem;">Gravadas no banco de dados — continuam salvas mesmo ao fechar ou atualizar a página. Já valem para as próximas recomendações.</span>',
      true
    );
  } else {
    _mostrarMsgAplicado(
      '<strong>❌ Não foi possível salvar no banco.</strong><br>' +
      '<span style="font-size:.82rem;">' + (res.erro || 'Erro desconhecido.') + '<br>Verifique sua conexão e tente novamente.</span>',
      false
    );
  }
}
...
async function _supaSalvarLoteParametros(cs) {
  ...
  var rows = cs.filter(function(c){ return c && c.nome; }).map(_montarRowParametro);
